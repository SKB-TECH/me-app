    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
          <Link to={'/'} className='flex items-center gap-2' onClick={()=>{
              ...
              <img src={logoben} className='w-32 h-32 object-contain  '/>
          </Link>
          ...
          <div className='sm:hidden flex flex-1 justify-end items-center'>
                    ...
                    <ul className='list-none flex flex-col gap-4 items-start justify-end'>
              {
                navLinks.map((link)=>(
                  <li key={link.id} 
                     className={`${active===link.title ? "text-white":"text-secondary"} font-poppins font-medieum text-[16px] cursor-pointer` }
                     onClick={()=>{
                      setToggle(!toggle);
